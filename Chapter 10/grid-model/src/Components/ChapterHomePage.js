...
const ChapterHomePage = (props) => {
  const [subChapters, setSubChapters] = useState();
  useEffect(() => {
    const fetchSubChapters = async () => {
      const subChapters = await getSubChapterNames(props.chapterNumber);
    ...
    const mapOfSubChapters = subChapters.map((subchapter) => {
      ...
      return (
        <li
          key={uuidv4()}
          className="square"
          onClick={() => props.handleSubChapterSelection(subChapName)}
        >
          <p> {subChapName}</p>
        </li>
      );
        ...
        <div>
          <header className="chapter-header">
            <h1 className="book-title">
              {props.chapterNumber}: {props.chapterName}
            </h1>
          </header>
